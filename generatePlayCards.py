...
def generate_qr_code(
        ...
        file_path: str,
        icon_path: str|None,
        icon_image_cache: dict[str, io.BytesIO] = {}
    ):
    qr = qrcode.QRCode(
        version=1,
...
def add_text_box(
    ...
    if not pd.isna(info['Artist']):
        ...
        artist_lines = textwrap.wrap(artist_text, width=int(len(artist_text) / page_canvas.stringWidth(artist_text, font_artist, font_size_artist) * (box_size - text_indent*2)))
    ...
    if not pd.isna(info['Title']):
        ...
        title_lines = textwrap.wrap(title_text, width=int(len(title_text) / page_canvas.stringWidth(title_text, font_title, font_size_title) * (box_size - text_indent*2)))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--icon", help="Path to icon to embed to QR Code, should not exceed 300x300px and using transparent background", required = False)
